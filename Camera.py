...
class Camera (Thread):
    def __init__(self):
        ...
        self.daemon = True
        self.width = 640 # use lower resolution for motion check
        self.height = 480
        """
        self.width = 1088 # use lower resolution for motion check
        self.height = 720
        """
        self.camera=PiCamera()
        self.camera.resolution=(self.width ,self.height)
        self.threshold=30 # how much must the color value (0-255) change to be considered a change
        self.minPixelsChanged = self.width * self.height * 2 / 100
    ...
    def photo (self,individu) :
        ...
        self.camera.capture('/home/pi/Desktop/Images/camera1'+name+individu+'.jpg')
        
    def film (self,individu):
        date_time=datetime.datetime.now()
        name= date_time.strftime("%m_%d_%Y__%H_%M_%S")
        self.camera.start_recording('/home/pi/Desktop/Images/camera1'+name+individu+'.h264')
        time.sleep(5)
