...
from USB_detecteur import Maintenance_USB,Redemarrage_post_maintenance
...
def init_peripheriques():
    ...
    return RFID,BARRIERE_IR,CAMERA,CAMERA2,CAPTEUR_PIR,HAUT_PARLEURS,CONFIGURATION,ENREGISTREMENT,LEDS,COMPTEUR,SERVOMOTEURS,WITTYPI,XBEE
...
def affectation_config(RFID,LEDS,CONFIGURATION,SERVOMOTEURS,WITTYPI):
    scenario, nom, emplacement_of, autorisation_recompense, probabilite_recompense,sans_punition,nb_ouverture_autorisee=CONFIGURATION.renvoie_config_general()
    ...
    facteur_division, portion_de_debut, phase_rfid, duree_par_portion,rfid_actif=CONFIGURATION.renvoie_config_pittags()
    ...
    RFID.set_pittag_choices(facteur_division, portion_de_debut,duree_par_portion,rfid_actif)
    ...
    SERVOMOTEURS.set_nb_ouverture_autorise(nb_ouverture_autorisee)
...
def traitement_scenarios(ARRET_USB,USB):
    ...
    RFID,BARRIERE_IR,CAMERA,CAMERA2,CAPTEUR_PIR,HAUT_PARLEURS,CONFIGURATION,ENREGISTREMENT,LEDS,COMPTEUR,SERVOMOTEURS,WITTYPI,XBEE=init_peripheriques()
    ...
    if(not(ARRET_USB.stop_peripherique)):
        ...
        #fusion des .csv
        ENREGISTREMENT.fusion_des_csv()
        #transmission des informations par l'Xbee
        XBEE.horaire_extinction=WITTYPI.horaire_extinction
        XBEE.envoie_fichier()
...
def main(argv) :
    try:
        ...
        traitement_scenarios(ARRET_USB,USB) 
